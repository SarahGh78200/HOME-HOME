...
public function dashboardGet()
{
    ...
    // 2. Vérifie que c'est bien un administrateur
    if (!isset($user['idRole']) || $user['idRole'] != 1) {
        $this->redirectToRoute('/'); // redirection vers l'accueil s   i non-admin
        return;
    }

    // 3. Si admin, on récupère les données
    $licences = Licence::getAllLicence();
    $users = User::getAllUsers();

    // 4. Envoie vers la vue dashboard
